...
public class RecipeWrapperAgglomeration implements IRecipeWrapper {
    ...
    public void getIngredients(IIngredients ing) {
        ...
        for (List<ItemStack> input : inputs) {
            if (input.size() == 1) {
                FluidStack fluidStack = FluidUtil.getFluidContained(input.get(0));
                if (fluidStack != null) {
                    fluidInputs.add(ImmutableList.of(fluidStack));
                }
            }
        }
        for (ItemStack output : outputs) {
            FluidStack fluidStack = FluidUtil.getFluidContained(output);
            if (fluidStack != null) {
                fluidOutputs.add(fluidStack);
            }
        }
        ing.setInputLists(VanillaTypes.FLUID, fluidInputs);
